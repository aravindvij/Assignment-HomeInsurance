...
public class RegistrationServices 
{
	...
	public RegistrationServices() throws Exception 
	{
		...
		con = DriverManager.getConnection("jdbc:mysql://localhost:3306/jspdb","root","admin1234");  
		stmt=con.createStatement();  
	}
	
	public boolean checkUsernameExists(String username) throws Exception
	{
		ResultSet rs=stmt.executeQuery("select username from JSPtable where username = \"" + username + "\"");  
		if(!rs.next())  
		{
			con.close();
			con = null;
	...
	{
		int i = stmt.executeUpdate("insert into JSPtable values (\""+user.getUsername() 
												        +"\",\"" + user.getPassword() 
												        +"\",\"" + user.getEmail() 
												        +"\"," + user.getPhone()+ ")");
		if(i == 1)
	...
	{
		ResultSet rs=stmt.executeQuery("select * from JSPtable where username = \"" + username + "\"");
		if(!rs.next())
		{
			con.close();
			con = null;
